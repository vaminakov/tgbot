...
| Тип PAM | Поведение |
|---|---|
| `session optional pam_tgbot.so` | Отправляет "🔑 Авторизован: user с IP" с кнопками **[Завершить сессию]**, **[Блокировать IP]** и **[Whois IP]** |
...
notify_exclude_users    = ["gitlab"]  # сервисные аккаунты — пропустить уведомления и 2FA
```
...
- Кнопка **Whois IP** выполняет RDAP-запрос по IP источника и отвечает страной, организацией и контактами регистранта. Отображается только если `PAM_RHOST` — валидный IP-адрес.
- `notify_exclude_users` подавляет уведомления о входе и 2FA для указанных пользователей (сервисные аккаунты, CI-runners). Установите `[]`, чтобы отключить исключения. Системный fallback `"unknown"` никогда не исключается, независимо от настроек.
